...
use crate::nanocloud::k8s::store::common::{
    ...
    value_file_path, with_resource_lock, DAEMONSET_PREFIX, K8S_KEYSPACE,
};
...
pub fn list_daemon_sets() -> Result<Vec<StoredDaemonSet>, Box<dyn Error + Send + Sync>> {
    ...
    let root = namespaced_root(DAEMONSET_PREFIX);
    ...
    for namespace_entry in namespace_entries {
        ...
        for daemonset_entry in daemonset_entries {
            ...
            let value_path = value_file_path(DAEMONSET_PREFIX, &namespace_name, &daemonset_name);
...
pub fn get_daemon_set(
    ...
    let value_path = value_file_path(DAEMONSET_PREFIX, &namespace_value, name);
    let raw = match fs::read_to_string(&value_path) {
        Ok(contents) => contents,
...
pub fn delete_daemonset(
    ...
    validate_resource_target("DaemonSet", name, namespace, None, None)?;
...
fn make_daemonset_key(namespace: Option<&str>, name: &str) -> String {
    namespaced_key(DAEMONSET_PREFIX, namespace, name)
}
